...
def slots_calendário(data, horário_ind):
    ...
    while slot_0 < slot_final:
        ...
        if not (slot_0.hour == 13 or (slot_0.hour < 13 and next_slot.hour > 14)): # Não são adicionados ao calendário slots durante o horário de almoço dos médicos (13:00-14:00)!!
            slot_válido = f'{slot_0.strftime('%H:%M')} - {next_slot.strftime('%H:%M')}'
            slots.append(slot_válido)
        slot_0 = next_slot
    return slots
...
def consultas_futuras(paciente_id):
    ...
    return sorted(cons_futuras, key= lambda cons: datetime.strptime(cons['data'], '%Y-%m-%d'), reverse= True)

# Cancelamento de Consultas Agendadas e Não Concluídas
...
def guardar_vacinação(inscrição): # O argumento recebido é o dicionário com as informações da inscrição!!
    ...
    if inscrição.get('certificados'): # Se o cidadão que realiza inscrição na campanha se encontrar registado como utente da plataforma será adicionado ao seu dicionário de informação um campo de certificados de vacinação prévios
        for p in pacientes:
