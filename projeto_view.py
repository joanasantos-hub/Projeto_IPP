...
def Novo_Reg_Layout():
    ...
    layout = [
        ...
        [sg.Text('Localidade *', font= ("Arial Rounded Mt Bold", 14), text_color= 'white', background_color= '#FF69B4', key= '-REG_LOCAL-'), sg.Combo(['São Martinho das Amoreiras',
            'Valverde', 'Albernoa', 'Santa Vitória', 'São Bento do Mato', 'Nossa Senhora de Machede'], key= '-LOCAL-')],
...
def Atualizar_Tab_Consultas(paciente_id): # Atualização do histórico de consultas imediatamente após marcação de nova consulta
    ...
    for cons in cons_atualizadas:
        ...
        contacto_med = cons['contacto_médico']

        atualizações.append([data, horário, nome, esp, contacto_med])
...
def Portal_Utente_Layout():
    ...
    for consulta in consultas:
        ...
        nome = info_med[1] + ' ' + info_med[2] # Concactenamos as duas strings com o primeiro e último nomes de cada médico especialista!
        contacto_med = consulta['contacto_médico']
        
        tab_cons.append([data, horário, nome, esp, contacto_med])
    ...
    headers_cons = ['Data', 'Horário', 'Médico Especialista', 'Especialidade', 'Contacto']
    ...
    certificados = [ # Tabela de certificados de vacinação (vacina, dose, data de administração)
        [sg.Table(values= tab_certificados, headings= headers_certificados, auto_size_columns= True, justification= 'centre', num_rows= 2, max_col_width= 50, font= ("Arial Rounded Mt Bold", 13), text_color= '#FF69B4', background_color= 'white', alternating_row_color= '#FCE5FC', key= '-TAB_VAC-' )]
    ]
...
def Cancelar_Cons_Layout(paciente_id):
    ...
    tab_futuras = []
    headers = ['Data', 'Horário', 'Médico Especialista', 'Especialidade', 'Contacto']
    ...
    for consulta in consultas_futuras:
        ...
        tab_futuras.append([data, horário, nome, esp, contacto_med])
...
def Campanha_Vac_Layout():
    ...
    layout = [
        ...
        [sg.Table(values= tab_localidades, headings= header_loc, auto_size_columns= False, justification= 'centre', num_rows= 4, col_widths=[20, 9], font= ("Arial Rounded Mt Bold", 14), text_color= '#FF69B4', background_color= 'white', alternating_row_color= '#FCE5FC', key= '-TAB_LOC-'), 
         sg.Table(values= tab_plano_vac, headings= header_plano, auto_size_columns= True, justification= 'centre', num_rows= 4, max_col_width= 35, font= ("Arial Rounded Mt Bold", 14), text_color= '#FF69B4', background_color= 'white', alternating_row_color= '#FCE5FC', key= '-TAB_PLANO-')],
...
def Estatísticas_Layout():
    ...
    def gráfico_barras(dist, título, xlabel):
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, '-SAIR-'):
            ...
            gráfico_barras(faixas_etárias,'Distribuição por Faixa Etárias','Faixas Etárias')
            ...
            gráfico_barras(vacinas,'Distribuição por Vacina Administrada','Vacinas Administradas')
